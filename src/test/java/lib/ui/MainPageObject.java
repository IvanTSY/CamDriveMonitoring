...
public class MainPageObject {
    ...
    public void clickElementToTheLeft(String locator, String error_message)  //клик по кнопке удалить статью для ios
    {
        if (driver instanceof AppiumDriver) {
            ...
            int point_to_click_x = (right_x)+5; // находим правый верзний угол
            int point_to_click_y = (upper_y)-5;
    ...
    public void clickEndPointOnTimeLine(String locator, String error_message)  //клик по кнопке удалить статью для ios
    {
        if (driver instanceof AppiumDriver) {
            WebElement element = this.waitForElementPresent(locator + "/..", error_message); // "/.." переходим на элемент выше
            int right_x = element.getLocation().getX();  // вычисляем правую точку
            ...
            int point_to_click_x = right_x - 100; // находим правый верзний угол
            int point_to_click_y = middle_y;
